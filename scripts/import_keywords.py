...
def run_import_sync(dry_run: bool = False):
    ...
    # --- Generate informational variants for tier-1 brands (better AI Overview hit rate) ---
    _insert_informational_variants(conn_str=settings.database_url)


INFORMATIONAL_TEMPLATES = [
    "best ways to save money on {brand} orders",
    "how to get discounts on {brand} India",
    "is {brand} offering any deals right now",
    "which coupon sites work best for {brand}",
]
...
def _insert_informational_variants(conn_str: str):
    variants = []
    for brand_key in TIER_1_BRANDS:
        ...
        for template in INFORMATIONAL_TEMPLATES:
            text = template.format(brand=display)
            variants.append((text, category, "Informational", 1, brand_key, True))

    if not variants:
        return

    log.info(f"Inserting {len(variants)} informational variants for {len(TIER_1_BRANDS)} tier-1 brands")
    with psycopg.connect(conn_str, row_factory=dict_row) as conn:
        ...
        conn.commit()
    log.info(f"Informational variants inserted.")
