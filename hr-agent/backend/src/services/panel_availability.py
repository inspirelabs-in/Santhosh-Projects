...
# ─── Legacy: consensus-based flow (kept for backward compat) ─────


async def record_panel_response(
    *,
    meeting_session_id: UUID,
    panel_email: str,
    selected_slot_indices: list[int],
) -> dict[str, Any]:
    """Legacy: record panel member's slot selections for consensus flow."""
    async with session_scope() as session:
        ms = await session.get(MeetingSession, meeting_session_id)
        if ms is None:
        ...
        responses: dict[str, list[int]] = neg.get("panel_responses", {})
        responses[panel_email] = selected_slot_indices

        neg["panel_responses"] = responses
        ms.negotiation_state = neg

        await log_audit(
            session,
            application_id=ms.application_id,
            action="panel_availability_response",
            actor=panel_email,
            details={
                "meeting_session_id": str(meeting_session_id),
                "selected_slots": selected_slot_indices,
