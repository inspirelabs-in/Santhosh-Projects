...
from src.db.repositories.assignment import get as get_assignment_row
...
async def parse_assignment(
    *,
    ...
    async with session_scope() as session:
        ...
        assignment_row = await get_assignment_row(session, application_id)
    ...
    # Prefer structured AssignmentRow content (generated by the AI and rendered into
    # the PDF) over the V1 role text field — the row has the exact problems that were
    # sent to the candidate, so the evaluator can check what was actually asked.
    if assignment_row and assignment_row.brief_md:
        assignment_brief_str = assignment_row.brief_md[:4000]
    else:
        assignment_brief_str = (role.assignment_brief or "")[:3000]

    problems_json_str = "(not available — fall back to assignment brief)"
    if assignment_row and assignment_row.problems:
        problems_json_str = json.dumps(assignment_row.problems, ensure_ascii=False)[:6000]

    assignment_rubric_json_str = "(not available)"
    if assignment_row and assignment_row.evaluation_rubric:
        assignment_rubric_json_str = json.dumps(assignment_row.evaluation_rubric, ensure_ascii=False)[:4000]
    ...
    prompt = compile_prompt(
        ...
        problems_json=problems_json_str,
        assignment_instructions=(role.assignment_instructions or "")[:2000],
        assignment_rubric_json=assignment_rubric_json_str,
